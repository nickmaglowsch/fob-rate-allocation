...
export class Tab5Page {
  ...
  MSC_qty: number
  maersk_qty: number
  CMA_CGM_qty_total: number
  hamburgSud_qty_total: number
  hapagLloyd_qty_total: number
  MSC_qty_total: number
  maersk_qty_total: number
  ...
  getArmadorQtyByName(armadorName) {

    switch (armadorName) {
        case "CMA_CGM":
            return this.CMA_CGM_qty_total
        case "MSC":
            return this.MSC_qty_total
        case "hamburgSud":
            return this.hamburgSud_qty_total
        case "hapagLloyd":
            return this.hapagLloyd_qty_total
        case "maersk":
            return this.maersk_qty_total
        default:
            return "invalid name"
    }
  ...
  allocContainerByName(armadorName, containersToAlloc) {

    switch (armadorName) {
      case "CMA_CGM":
        this.CMA_CGM_qty_total = this.CMA_CGM_qty_total - containersToAlloc;
        break;
      case "MSC":
        this.MSC_qty_total = this.MSC_qty_total - containersToAlloc;
        break;
      case "hamburgSud":
        this.hamburgSud_qty_total = this.hamburgSud_qty_total - containersToAlloc;
        break;
      case "hapagLloyd":
        this.hapagLloyd_qty_total = this.hapagLloyd_qty_total - containersToAlloc;
        break;
      case "maersk":
        this.maersk_qty_total = this.maersk_qty_total - containersToAlloc;
        break;
    }
  }
  ...
  customLimit() {
    ...
    this.CMA_CGM_qty_total = Math.ceil(total * (this.CMA_CGM_qty / 100))
    this.hapagLloyd_qty_total = Math.ceil(total * (this.hapagLloyd_qty / 100))
    this.hamburgSud_qty_total = Math.ceil(total * (this.hamburgSud_qty / 100))
    this.maersk_qty_total = Math.ceil(total * (this.maersk_qty / 100))
    this.MSC_qty_total = Math.ceil(total * (this.MSC_qty / 100))
  }

  createRates() {
    this.chartFactory.destroyChart("armadores")
    this.customLimit()
    this.total = 0
    this.totalReal = 0
    this.createTotal()
    ...
    dataSet_.forEach(data => {
      ...
      if (this.onlyOneShipper(thisPrice)) {
        ...
        this.allocContainerByName(singleShipper.name, Number(data.qty))
    ...
    dataSet_.forEach(data => {
      ...
      this.allocContainerByName(bestPrice.name, Number(data.qty))
    ...
    this.rates.forEach(data => this.total = this.total + data.totalPrice)
    this.createChart()
  ...
  findBestPrice(priceList: PriceList, containerQty) {
    ...
    for (const key in priceList) {
      if (priceList.hasOwnProperty(key)) {
        ...
        if (typeof element === 'object') {
          if (element.value != 0) {
            if (this.getArmadorQtyByName(element.name) >= containerQty) {
              if (element.value < best.value) {
                best = element
              }
            }
